...
def cycle_power(some_arr: list, power: int) -> list:
    ...
    Возвращает его измененным, таким же или e
    """
    if power == 0:
        return ['e']
    if power == 1:
...
def solve(arr_a: list, arr_b: list, power_a: int, power_b: int):
    """
    Выводит решение и возвращает массив с финальной перестановкой
    """
    ...
    for phi in phi_a:
        print(
            f"Цикл {phi} длины {len(phi)} получит степень {power_a%len(phi)}", '\n')
        phi_a_reformed.append(cycle_power(phi, power_a % len(phi)))

    print(f'Cтепень b равна {power_b}:', '\n')
    for phi in phi_b:
        print(
            f"Цикл {phi} длины {len(phi)} получит степень {power_b%len(phi)}" '\n')
